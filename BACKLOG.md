...
| **49 OCR-zerrissene Überschriften** in den Druck-Scans (`KIN DH EITSERIN N ERU NGEN`) | niedrig | **Kapitel-/Abschnittstitel, keine Regelbegriffe** — für sie existiert kein Wörterbuch-Beleg. `repariere_2014_namen` verlangt einen Beleg; ohne ihn wäre die Reparatur geraten (Regel 1). Die belegbaren 69 sind repariert. *Zahl korrigiert am 01.08.2026:* Hier stand 51, gemeldet wurden 91 — die Differenz war **Rauschen**. 42 der Treffer sind alphabetische **Registerköpfe** aus DDB-Büchern (`B | Monsters`, `Spells J`), also gar keine Risse; sie überdeckten die echten Befunde. Die Heuristik nimmt sie jetzt aus, und `config/qualitaet_basis.json` hält den Stand je Quelle fest — steigt eine Zahl, bricht `admin check` (§Wächter unten) |
