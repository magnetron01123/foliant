...
OAuth; `make url SERVICE=foliant` im Projekt „Ingress Gateway“ druckt sie. Das abschließende
...
außen gehört einem fremden Stack (`~/ingress-gateway`), den sich mehrere Projekte
...
| Teil | Container | Was es trägt | Foliants Berührpunkt |
|---|---|---|---|
...
1. **Geheimpfad** — die URL ist der Schlüssel. **Wer ihn hält, entscheidet `FOLIANT_ZUGANG`:**
   ...
   „Ingress Gateway“ — das betrifft dann **alle** MCP-Server des Geräts.
...
`docker compose up -d --build`. Mit umziehen müssen der fremde Stack `~/ingress-gateway` (beide Tunnel plus Router) und seine Netze; Tunnel-Token und Connector-URL bleiben, der
...
| **Kein eigener Tunnel-Connector mehr** (26.08.2026) | Foliants `cloudflared` war ein *zweiter* Connector desselben Tunnels „mcp" — dieselbe Verbindung, zweimal betrieben, aus zwei Repositorys gepflegt. Die Tunnel liegen jetzt im fremden Stack `~/ingress-gateway`, getrennt nach MCP und Website: ein Connector erreicht nur Container in seinen eigenen Netzen, ein Website-Deploy kann also keinen MCP-Server mitreißen. Foliants Repository trägt dafür kein Tunnel-Token mehr |
...
- **Der Containername `foliant-mcp` IST die Route.** Der geteilte Router leitet
  ...
  Router tatsächlich erreicht, zeigt `make services` im Projekt „Ingress Gateway".
- **Die externen Netze müssen VOR dem Stack existieren.** `mcp-net` und `web-edge` sind
  ...
  Angelegt werden sie mit `make net` im Projekt „Ingress Gateway".
