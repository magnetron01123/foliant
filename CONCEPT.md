...
| **Prompt-Caching formt die Anfrage an drei Stellen — der Eval bleibt außen vor** (09.08.2026) | `system_cachen=True` (nur der Bot) setzt einen festen Breakpoint auf den System-Block, einen request-weiten Breakpoint für den wachsenden Teil (Tool-Ergebnisse, Thread-Verlauf), und entzieht dem Modell die Werkzeuge in der Schlussrunde per `tool_choice: none` statt sie wegzulassen. Der dritte Punkt ist der unauffälligste: Werkzeuge stehen **ganz oben** im Präfix (tools → system → messages), wer dort etwas ändert, verwirft alles dahinter — das Weglassen entzog also nicht nur die Werkzeuge, es warf den kompletten Cache weg und schrieb einen nie wieder gelesenen neuen. Der Eval fährt weiter ohne all das: seine Anfrageform ist die Messgrundlage (`SchleifenErgebnis.verbrauch` zählt die Token-Felder mit, weil ein verfehlter Cache sich **nirgends** meldet — er kostet nur still den vollen Preis) |
| **Cache-Lebensdauer 1 Stunde statt der voreingestellten 5 Minuten** (09.08.2026) | Nicht geschätzt, sondern am Abfrage-Protokoll gemessen: 1997 Aufrufe (26.07.–09.08.2026), zu 113 Fragen gebündelt, ergeben einen **Median von 14 Minuten** zwischen zwei Fragen; nur 27–31 % der Lücken liegen unter 5 Minuten, 59–71 % unter einer Stunde. Der voreingestellte Cache wäre also meistens abgelaufen, bevor die nächste Frage kommt — man zahlte den Schreib-Aufschlag und läse nie. Die Stunde kostet beim Schreiben 2× statt 1,25× und ist deshalb bei der **ersten** Frage teurer; ab der zweiten ist sie billiger, weil jeder Treffer die Frist kostenlos verlängert. Gegenprobe ohne die Eval-Tage und nur auf den jüngsten Tagen ändert das Bild nicht |
