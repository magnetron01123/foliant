...
außen gehört einem fremden Stack (`~/cloudflare-tunnel`), den sich mehrere Projekte
...
`docker compose up -d --build`. Mit umziehen müssen der fremde Stack `~/cloudflare-tunnel` (beide Tunnel plus Router) und seine Netze; Tunnel-Token und Connector-URL bleiben, der
